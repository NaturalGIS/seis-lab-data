...
from ...db.queries.workflowstages import get_workflow_stage_by_english_name
from ...schemas import (
...
class _BaseForm(StarletteForm):
    ...
    async def check_if_english_name_is_unique(
        ...
        disregard_id: identifiers.WorkflowStageId | None = None,
    ):
        """Check if current english name is already used by another workflow stage.
        ...
        category, in which case the resource itself should be disregarded, as
        it is not a conflict for a workflow stage to have the same english name
        as itself.
        """
        error_message = _("There is already a workflow stage with this english name")
        if (parsed_name := self._parse_name()) is None:
            return
        if candidate := await get_workflow_stage_by_english_name(
            session, parsed_name["en"]
        ):
            if disregard_id:
                if identifiers.WorkflowStageId(candidate.id) != disregard_id:
    ...
    async def get_validated_form_instance(
        cls, request: Request, disregard_id: identifiers.WorkflowStageId | None = None
    ):
        """Performs full validation of an workflow_stage-related form.
...
class WorkflowStageCreateForm(_BaseForm):
...
class WorkflowStageUpdateForm(_BaseForm):
    def validate_with_schema(self):
        try:
            stage_schemas.WorkflowStageUpdate(
                name=self.name.data,
            )
        except pydantic.ValidationError as exc:
            incorporate_schema_validation_errors_into_form(exc.errors(), self)
